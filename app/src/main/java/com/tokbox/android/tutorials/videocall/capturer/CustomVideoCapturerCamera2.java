...
class CustomVideoCapturerCamera2 extends BaseVideoCapturer implements BaseVideoCapturer.CaptureSwitch {
    ...
    public ByteBuffer plane0, plane1, plane2;
    public int rowStride0, rowsStride1, rowsStride2;
    public int pixelStride0, pixelStride1, pixelStride2;
   ...
   /* private ImageReader.OnImageAvailableListener frameObserver = new ImageReader.OnImageAvailableListener() {
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            if (plane0 != null) {
                ...
                planes = frame.getPlanes();
                width = frame.getWidth();
                height = frame.getHeight();
                if (planes.length > 0) {
                    plane0 = frame.getPlanes()[0].getBuffer();
                    pixelStride0 = frame.getPlanes()[0].getPixelStride();
                    rowStride0 = frame.getPlanes()[0].getRowStride();
                }

                if (planes.length > 1) {
                    plane1 = frame.getPlanes()[1].getBuffer();
                    pixelStride1 = frame.getPlanes()[1].getPixelStride();
                    rowsStride1 = frame.getPlanes()[1].getRowStride();
                }
                if (planes.length > 2) {
                    plane2 = frame.getPlanes()[2].getBuffer();
                    pixelStride1 = frame.getPlanes()[2].getPixelStride();
                    rowsStride2 = frame.getPlanes()[2].getRowStride();
                }
            }

            Log.d("XD", "new image from camera");
            if ((plane0 == null)
                    || (plane1 == null)
                    || (plane2 == null)) {
                Log.d(LOG_TAG, "onImageAvailable frame provided has no image data");
